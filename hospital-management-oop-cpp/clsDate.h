...
public:
	...
	static string Clock12Hours()
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		return to_string(now->tm_hour - 12) + ":" + to_string(now->tm_min + 1) + ":" + to_string(now->tm_sec);
	}
	static string Clock24Hours()
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		return to_string(now->tm_hour + 1) + ":" + to_string(now->tm_min + 1) + ":" + to_string(now->tm_sec);
	}
	static string GetSystemDateTimeString()
	{
		time_t t = time(0);
		tm* now = localtime(&t);
		short Day = 0, Month = 0, Year = 0, Hour = 0, Minute = 0, Second = 0;
		Year = now->tm_year + 1900;
		Month = now->tm_mon + 1;
		Day = now->tm_mday;
		Hour = now->tm_hour;
